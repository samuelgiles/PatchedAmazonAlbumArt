...
module AmazonAlbumArt
  ...
  class Client
    ...
    def search(artist, album, sizes = [:swatch, :small, :thumbnail, :tiny, :medium, :large])
      ...
      #if defined? @worker.parameters.delete_if=()
      #@worker.parameters.delete_if { |k,v| %w{IdType ResponseGroup ItemId}.include? k }

      if @worker.parameters[:IdType].present?
      @worker.parameters.delete :IdType
      end
      if @worker.parameters[:ResponseGroup].present?
      @worker.parameters.delete :ResponseGroup
      end
      if @worker.parameters[:ItemId].present?
      @worker.parameters.delete :ItemId
      end
      ...
      results.map("Item") do |match|
        ...
        if @worker.parameters[:SearchIndex].present?
        @worker.parameters.delete :SearchIndex
        end
        if @worker.parameters[:Title].present?
        @worker.parameters.delete :Title
        end
        if @worker.parameters[:Artist].present?
        @worker.parameters.delete :Artist
        end
        #end
        ...
        #@worker.parameters[:SearchIndex].delete 
